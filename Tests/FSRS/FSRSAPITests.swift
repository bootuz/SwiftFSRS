...
struct FSRSAPITests {
    private func createFSRS(enableFuzz: Bool = false, enableShortTerm: Bool = true) -> FSRS<
        TestCard
    > {
    ...
    func testRepeatCalculatesStability() throws {
        ...
        let goodStability = try #require(recordLog[.good]).card.stability
        let easyStability = try #require(recordLog[.easy]).card.stability
    ...
    func testRepeatCalculatesDifficulty() throws {
        ...
        let goodDifficulty = try #require(recordLog[.good]).card.difficulty
        let easyDifficulty = try #require(recordLog[.easy]).card.difficulty
