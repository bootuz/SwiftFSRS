...
struct IntegrationTests {
    ...
    func testConsistentStudySchedule() throws {
        ...
        for i in 3...7 {
            if card.scheduledDays > 0 {
                now = Calendar.current.date(byAdding: .day, value: Int(card.scheduledDays), to: card.lastReview ?? now)!
            } else {
                now = Calendar.current.date(byAdding: .day, value: 1, to: now)!
            }
    ...
    func testForgettingAndRelearning() throws {
        ...
        // Forget the card
        now = Calendar.current.date(byAdding: .day, value: 30, to: now)!
    ...
    func testLongTermRetention() throws {
        ...
        while totalDays < 365 && card.reps < 20 {
            ...
            now = Calendar.current.date(byAdding: .day, value: daysToAdd, to: now)!
            totalDays += daysToAdd
        }
    ...
    func testCrammingVsSpacedRepetition() throws {
        ...
        for _ in 1...10 {
            ...
            if spacedCard.scheduledDays > 0 {
                spacedDate = Calendar.current.date(byAdding: .day, value: Int(spacedCard.scheduledDays), to: spacedDate)!
            } else {
                spacedDate = Calendar.current.date(byAdding: .day, value: 1, to: spacedDate)!
            }
        }
    ...
    func testImportExistingCard() throws {
        ...
        let importedCard = TestCard(
            question: "Imported Q",
            answer: "Imported A",
            due: Calendar.current.date(byAdding: .day, value: -5, to: now)!,
            state: .review,
    ...
    func testRetrievabilityOverTime() throws {
        ...
        for days in 1...30 {
            let checkDate = Calendar.current.date(byAdding: .day, value: days, to: baseDate)!
