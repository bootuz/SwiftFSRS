...
struct ParameterTests {
    ...
    func testGenerateDefaultParameters() {
        ...
        #expect(generated.w.count == 21) // FSRS-6 has 21 parameters
    ...
    func testRequestRetentionRange() throws {
        let f: FSRS<TestCard> = fsrs(params: PartialFSRSParameters(requestRetention: 0.9))
        
        #expect(f.parameters.requestRetention > 0)
        #expect(f.parameters.requestRetention <= 1)
    }
    
    @Test("Maximum interval must be positive")
    func testMaximumIntervalPositive() {
        let f: FSRS<TestCard> = fsrs(params: PartialFSRSParameters(maximumInterval: 365))
    ...
    func testWeightArrayLength() {
        ...
        #expect(f.parameters.w.count == 21)
    }
    ...
    func testLearningStepsAffectScheduling() throws {
        ...
        _ = try f.repeat(card: card, now: Date())
        
        // Learning steps should be reflected in the scheduling
        #expect(f.parameters.learningSteps == steps)
    }
    ...
    func testRelearningStepsAffectScheduling() throws {
        ...
        #expect(f.parameters.relearningSteps == steps)
    }
    ...
    func testFuzzDisabledConsistency() throws {
        ...
        let result1 = try f.next(card: card, now: now, grade: .good)
        let result2 = try f.next(card: card, now: now, grade: .good)
    ...
    func testUpdateParametersAffectsScheduling() throws {
        var f: FSRS<TestCard> = fsrs(params: PartialFSRSParameters(requestRetention: 0.9))
        
        let card = TestCard(question: "Test", answer: "Test")
        _ = try f.next(card: card, now: Date(), grade: .good)
        
        // Update parameters
        var newParams = f.parameters
        newParams.requestRetention = 0.7
        f.parameters = newParams
        
        _ = try f.next(card: card, now: Date(), grade: .good)
        
        // Different retention should affect intervals
        #expect(f.parameters.requestRetention == 0.7)
    }
    ...
    func testSmallMaximumInterval() {
        ...
        #expect(f.parameters.maximumInterval == 7)
    }
