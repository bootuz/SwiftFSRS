...
public enum AleaSeed: Sendable {
    ...
    fileprivate var hashableValue: String {
        ...
        case .double(let d): return String(describing: d)
        case .int(let i): return String(describing: i)
        }
    }
}
...
private final class Alea: @unchecked Sendable {
    ...
    func next() -> Double {
        let t = 2091639.0 * s0 + Double(c) * 2.3283064365386963e-10 // 2^-32
        s0 = s1
        s1 = s2
        c = UInt32(t)
        s2 = t - Double(c)
    ...
    var state: AleaState {
        get {
            return AleaState(c: c, s0: s0, s1: s1, s2: s2)
        }
        set {
            c = newValue.c
            s0 = newValue.s0
...
public struct AleaPRNG: Sendable {
    ...
    public mutating func int32() -> Int32 {
        return Int32(alea.next() * 0x100000000)
    }
